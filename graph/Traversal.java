 ...
 *  fringe, and the visit method may throw a RejectExcption to
...
public class Traversal<VLabel, ELabel> {
    ...
    public void traverse(Graph<VLabel, ELabel> G,
        ...
        Comparator<Graph<VLabel, ELabel>.Vertex> newOrder =
            new Comparator<Graph<VLabel, ELabel>.Vertex>() {
                @Override
                public int compare(Graph<VLabel, ELabel>.Vertex o1,
                    Graph<VLabel, ELabel>.Vertex o2) {
                    return order.compare(o1.getLabel(), o2.getLabel());
                }
            };

        PriorityQueue<Graph<VLabel, ELabel>.Vertex> fringe =
            new PriorityQueue<Graph<VLabel, ELabel>
            .Vertex>(G.vertexSize(), newOrder);
        fringe.add(v);
        while (!fringe.isEmpty()) {
            v = fringe.remove();
            if (_markedSet.contains(v) && !_postedSet.contains(v)) {
                postVisit(v);
                _postedSet.add(v);
            } else {
                try {
                ...
                for (Graph<VLabel, ELabel>.Edge edg: G.outEdges(v)) {
                    Graph<VLabel, ELabel>.Vertex ver = edg.getV(v);
                    if (!_markedSet.contains(ver)) {
                        try {
                            preVisit(edg, ver);
                        } catch (RejectException e) {
                            continue;
    ...
    public void depthFirstTraverse(Graph<VLabel, ELabel> G,
        ...
        Stack<Graph<VLabel, ELabel>.Edge> edges;
        fringe.add(v);
        while (!fringe.isEmpty()) {
            v = fringe.pop();
            if (marked(v)) {
                ...
                try {
                    ...
                    _finalVertex = v; return;
                } catch (RejectException error) {
                    continue;
                }
                mark(v);
                fringe.add(v);
                edges = new Stack<Graph<VLabel, ELabel>.Edge>();
                for (Graph<VLabel, ELabel>.Edge edg: G.outEdges(v)) {
                    Graph<VLabel, ELabel>.Vertex ver = edg.getV1();
                    try {
                        if (!marked(ver)) {
                ...
                while (true) {
                    if (!edges.empty()) {
                        Graph<VLabel, ELabel>.Vertex vert
                            = edges.peek().getV1();
                        if (!fringe.contains(vert)) {
                            edges.pop(); fringe.push(vert);
                        }
                    } else { break; }
                }
            }
        ...
        while (!fringe.isEmpty()) {
            try {
                ...
                if (marked(v)) {
                    ...
                    for (Graph<VLabel, ELabel>.Edge edg: G.outEdges(v)) {
                        Graph<VLabel, ELabel>.Vertex ver = edg.getV(v);
                        if (!marked(ver)) { preVisit(edg, v); }
                        if (!fringe.contains(ver)) { fringe.add(ver); }
                    }
                    mark(v); fringe.add(v);
                }
            } catch (StopException | RejectException e) {
                continue;
            }
        }
    }
    ...
    /** Return if the vertex V is marked. */
    private boolean marked(Graph<VLabel, ELabel>.Vertex v) {
        return _markedSet.contains(v);
    }
    /** mark the vertex V. */
    ...
    private void cleanMark() {
        _markedSet = new HashSet<Graph<VLabel, ELabel>.Vertex>();
    }
    /** The Set of marked. */
    private HashSet<Graph<VLabel, ELabel>.Vertex> _markedSet =
        new HashSet<Graph<VLabel, ELabel>.Vertex>();
    /** The Set of posted. */
    private HashSet<Graph<VLabel, ELabel>.Vertex> _postedSet;
